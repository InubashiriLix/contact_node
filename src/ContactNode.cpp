...
public:
  ContactNode() : Node("contact") {
    ...
    __rx_timer__ = this->create_wall_timer(
        100ms, std::bind(&ContactNode::rx_timer_callback, this));
  ...
  void rx_timer_callback() {
    ...
    quaternion_msg.x = comm.rx_struct_.q[0];
    quaternion_msg.y = comm.rx_struct_.q[1];
    quaternion_msg.z = comm.rx_struct_.q[2];
    quaternion_msg.w = comm.rx_struct_.q[3]; // 写到打符pre
    ...
    autoaim_status_publishing_msg.yaw = comm.rx_struct_.pitch;
    autoaim_status_publishing_msg.pitch = comm.rx_struct_.yaw;
    RCLCPP_INFO(this->get_logger(), "Published pitch: %f, yaw: %f", pitch_temp,
                yaw_temp);
    ...
    mode_msg.data =
        comm.rx_struct_.vision_mode; // NOTE: 可以替换为任何你想要的整数值
    // 发布消息
    _mode_pub_->publish(mode_msg);
    RCLCPP_INFO(this->get_logger(), "Published integer: %d", mode_msg.data);

    // timestamp
    uint32_t timestamp = comm.rx_struct_.timestamp;
    // TODO: WHAT TO DO WITH THE TIMESTAMP

    // publish the enemy hp
    uint8_t side = comm.rx_struct_.current_side_color;
    sentry_msgs::msg::RobotHP enemy_hp_msg;
    auto enemy_hp = comm.rx_struct_.enemy_hp;
    ...
    bullet_remaining_msgs.remaining_17mm_num =
        comm.rx_struct_.bullet_remaining_info[0];
    bullet_remaining_msgs.remaining_42mm_num =
        comm.rx_struct_.bullet_remaining_info[1];
    bullet_remaining_msgs.remaining_coin_num =
        comm.rx_struct_.bullet_remaining_info[2];
    _bullet_remain_pub_->publish(bullet_remaining_msgs);
    RCLCPP_INFO(this->get_logger(), "Published bullet remaining: %d, %d, %d",
                comm.rx_struct_.bullet_remaining_info[0],
                comm.rx_struct_.bullet_remaining_info[1],
                comm.rx_struct_.bullet_remaining_info[2]);
    ...
    field_events_msgs.supplier_1_occupation = comm.rx_struct_.field_events[0];
    field_events_msgs.supplier_2_occupation = comm.rx_struct_.field_events[1];
    field_events_msgs.supplier_3_occupation = comm.rx_struct_.field_events[2];
    field_events_msgs.power_rune_activation_point_occupation =
        comm.rx_struct_.field_events[3];
    field_events_msgs.small_power_rune_activation_status =
        comm.rx_struct_.field_events[4];
    field_events_msgs.big_power_rune_activation_status =
        comm.rx_struct_.field_events[5];
    field_events_msgs.r2b2_ground_occupation = comm.rx_struct_.field_events[6];
    field_events_msgs.r3b3_ground_occupation = comm.rx_struct_.field_events[7];
    field_events_msgs.r4b4_ground_occupation = comm.rx_struct_.field_events[8];
    field_events_msgs.base_has_shield = comm.rx_struct_.field_events[9];
    field_events_msgs.outpost_alive = comm.rx_struct_.field_events[10];

    _field_events_pub_->publish(field_events_msgs);

    // game satatus
    sentry_msgs::msg::GameStatus game_status_msgs;
    game_status_msgs.game_type = comm.rx_struct_.game_type;
    game_status_msgs.game_progress = comm.rx_struct_.game_progress;
    game_status_msgs.stage_remain_time = comm.rx_struct_.state_remain_time;
    game_status_msgs.sync_time_stamp = comm.rx_struct_.sync_time_stamp;
    _game_status_pub_->publish(game_status_msgs);
    RCLCPP_INFO(this->get_logger(), "Published game status: %d, %d, %d",
                comm.rx_struct_.game_type, comm.rx_struct_.game_progress,
                comm.rx_struct_.state_remain_time);

    // game result
    sentry_msgs::msg::GameResult game_result_msgs;
    game_result_msgs.winner = comm.rx_struct_.winner;
    _game_result_pub_->publish(game_result_msgs);
    RCLCPP_INFO(this->get_logger(), "Published game result: %d",
                comm.rx_struct_.winner);

    // robot buff
    sentry_msgs::msg::RobotBuff robot_buffs_msgs;
    robot_buffs_msgs.robot_replenishing_blood = comm.rx_struct_.robot_buffs[0];
    robot_buffs_msgs.shooter_cooling_acceleration =
        comm.rx_struct_.robot_buffs[1];
    robot_buffs_msgs.robot_defense_bonus = comm.rx_struct_.robot_buffs[2];
    robot_buffs_msgs.robot_attack_bonus = comm.rx_struct_.robot_buffs[3];
    RCLCPP_INFO(this->get_logger(), "Published robot buff: %d, %d, %d, %d",
                comm.rx_struct_.robot_buffs[0], comm.rx_struct_.robot_buffs[1],
                comm.rx_struct_.robot_buffs[2], comm.rx_struct_.robot_buffs[3]);

    // robot position
    sentry_msgs::msg::RobotPosition robot_position_msgs;
    robot_position_msgs.x = comm.rx_struct_.robot_position[0];
    robot_position_msgs.y = comm.rx_struct_.robot_position[1];
    robot_position_msgs.z = comm.rx_struct_.robot_position[2];
    robot_position_msgs.yaw = comm.rx_struct_.robot_position[3];

    _robot_position_pub_->publish(robot_position_msgs);
    RCLCPP_INFO(
        this->get_logger(), "Published robot position: %f, %f, %f, %f",
        comm.rx_struct_.robot_position[0], comm.rx_struct_.robot_position[1],
        comm.rx_struct_.robot_position[2], comm.rx_struct_.robot_position[3]);

    // robot status
    sentry_msgs::msg::RobotStatus robot_status_msgs;
    robot_status_msgs.robot_id = comm.rx_struct_.robot_status_0[0];
    robot_status_msgs.robot_level = comm.rx_struct_.robot_status_0[1];
    robot_status_msgs.remain_hp = comm.rx_struct_.robot_status_1[0];
    robot_status_msgs.max_hp = comm.rx_struct_.robot_status_1[1];
    robot_status_msgs.shooter_17mm_id1_cooling_rate =
        comm.rx_struct_.robot_status_1[2];
    robot_status_msgs.shooter_17mm_id1_cooling_limit =
        comm.rx_struct_.robot_status_1[3];
    robot_status_msgs.shooter_17mm_id1_speed_limit =
        comm.rx_struct_.robot_status_1[4];
    robot_status_msgs.shooter_17mm_id2_cooling_rate =
        comm.rx_struct_.robot_status_1[5];
    robot_status_msgs.shooter_17mm_id2_cooling_limit =
        comm.rx_struct_.robot_status_1[6];
    robot_status_msgs.shooter_17mm_id2_speed_limit =
        comm.rx_struct_.robot_status_1[7];
    robot_status_msgs.chassis_power_limit = comm.rx_struct_.robot_status_1[11];
    robot_status_msgs.gimbal_power_output = comm.rx_struct_.robot_status_1[12];
    robot_status_msgs.chassis_power_output = comm.rx_struct_.robot_status_1[13];
    robot_status_msgs.shooter_power_output = comm.rx_struct_.robot_status_1[14];

    // client command
    sentry_msgs::msg::ClientCommand client_command_msgs;
    client_command_msgs.target_position_x =
        comm.rx_struct_.command_target_position[0];
    client_command_msgs.target_position_y =
        comm.rx_struct_.command_target_position[1];
    client_command_msgs.target_position_z =
        comm.rx_struct_.command_target_position[2];
    client_command_msgs.keyboard_key_pressed =
        comm.rx_struct_.keyboard_key_pressed;
    client_command_msgs.target_robot_id =
        comm.rx_struct_.command_target_robot_id;
    ...
    client_receive_msgs.target_position_x =
        comm.rx_struct_.receive_target_position[0];
    client_receive_msgs.target_position_y =
        comm.rx_struct_.receive_target_position[1];
    client_receive_msgs.target_robot_id =
        comm.rx_struct_.receive_target_robot_id;
